...
from file_management import file_management  # ファイル管理モジュールをインポート
from data_processing import data_processing  # データ処理モジュールをインポート
from keyword_management import keyword_management  # キーワード管理モジュールをインポート
...
# ポジティブ・ネガティブキーワードを保存するファイルパス
POSITIVE_KEYWORDS_FILE = 'positive_keywords.json'
NEGATIVE_KEYWORDS_FILE = 'negative_keywords.json'

# 無効な単語を判定する関数
def is_invalid_word(word):
    if len(word) == 1 or word.isdigit():
        return True
    if re.search('[\u4E00-\u9FFF]', word) and re.search('[\u3040-\u309F]', word):
        return True
    if all(re.match('[\u3040-\u309F]', char) for char in word):
        return True
    return False
...
def extract_nouns_verbs_sudachi(text):
    ...
    for token in tokens:
        ...
        word = token.surface()
        if pos in ['名詞', '動詞'] and not is_invalid_word(word):
            words.append(word)
    return words
...
@app.route('/proceed_analysis', methods=['POST'])
...
@app.route('/store_keyword_counts', methods=['GET', 'POST'])
...
@app.route('/keyword_sentences/<keyword>')
...
@app.route('/store_analysis', methods=['GET', 'POST'])
def store_analysis():
    # 店舗ごとの頻出単語を抽出
...
@app.route('/store_analysis_save', methods=['POST'])
...
@app.route('/store_analysis_page/<store_name>')
...
@app.route('/store_analysis/<store_name>/<word>', methods=['GET'])
...
# キーワードをファイルに保存する関数
def save_keyword_to_file(word, filename):
    file_path = os.path.join(os.getcwd(), filename)
...
# キーワードをファイルから削除する関数
# ポジティブキーワードを追加する関数
def add_positive_keyword(word):
    add_keyword_to_file('positive_keywords.txt', word)

# ネガティブキーワードを追加する関数
def add_negative_keyword(word):
    add_keyword_to_file('negative_keywords.txt', word)

# ポジティブキーワードを削除する関数
def delete_positive_keyword(word):
    delete_keyword_from_file('positive_keywords.txt', word)

# ネガティブキーワードを削除する関数
def delete_negative_keyword(word):
    delete_keyword_from_file('negative_keywords.txt', word)

# キーワードをファイルに追加する共通関数
def add_keyword_to_file(filename, word):
    file_path = os.path.join(os.getcwd(), filename)
    with open(file_path, 'a', encoding='utf-8') as file:
        file.write(f'{word}\n')

# キーワードをファイルから削除する共通関数
def delete_keyword_from_file(filename, word):
    file_path = os.path.join(os.getcwd(), filename)
    try:
        with open(file_path, 'r', encoding='utf-8') as file:
...
@app.route('/manage_keywords', methods=['POST', 'GET'])
def manage_keywords():
    if request.method == 'POST':
    ...
    return render_template('manage_keywords.html', positive_keywords=positive_keywords, negative_keywords=negative_keywords)
...
# 解析結果ページのエンドポイント
@data_processing.route('/analysis', methods=['POST'])
def analysis():
    # ファイルの保存先ディレクトリ
    output_folder = 'filtered_emails'
    non_commercial_file_path = os.path.join(output_folder, 'non_commercial_emails.csv')
    ...
    for word in selected_words:
        word_df = df[df['内容'].str.contains(word, na=False)]
        store_keyword_counts[word] = {
            'filtered_stores': word_df['子要素'].value_counts().to_dict(),
            'below_3_stores': len(word_df[word_df['子要素'].value_counts() < 3]),
            'below_3_count': word_df[word_df['子要素'].value_counts() < 3].shape[0]
        }
        total_counts[word] = word_df.shape[0]

    return render_template('store_keyword_counts.html', store_keyword_counts=store_keyword_counts, total_counts=total_counts)